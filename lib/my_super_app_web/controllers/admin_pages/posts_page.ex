defmodule MySuperAppWeb.PostsPage do
  import Surface
  ...
  alias Moon.Design.Form.{TextArea, Input, Field}
  alias Moon.Icon
  ...
  def mount(_params, session, socket) do
    ...
    {:ok,
     assign(socket,
       ...
       limit: 10,
       sort: [name: "ASC"],
       total_pages: Blog.get_all_posts() |> Enum.count() |> CasinosAdmins.page_count(10),
       filter: "",
  ...
  def handle_event("open_add_post_drawer", _, socket) do
    Drawer.open("add_post_drawer")

    {:noreply,
     assign(socket,
       add_post_drawer_active: true,
       form: to_form(Post.changeset(%Post{}, %{}))
     )}
  end

  def handle_event("add_validate", %{"post" => params}, socket) do
  ...
  def handle_event("add_post", %{"post" => post_params}, socket) do
    tag_names = String.split(post_params["tagNames"])
    tags = Enum.map(tag_names, fn name -> Blog.get_tag_by_name(name) end)
    ...
    case Blog.create_post(post_params) do
      ...
      {:error, _reason} ->
        {:noreply,
         assign(socket |> put_flash(:error, "Failed to create post!"),
           add_post_drawer_active: false
         )}
    end
  end
  ...
  def handle_event("close_add_drawer_by_click", _, socket) do
    Drawer.close("add_post_drawer")
    {:noreply, assign(socket, add_post_drawer_active: false)}
  end

  def handle_event("close_add_post_drawer", _, socket) do
    {:noreply, assign(socket, form: to_form(Post.changeset(%Post{}, %{})))}
  end

  def handle_event("row_click", %{"selected" => selected}, socket) do
    post = Blog.get_post(selected)
    {:noreply, assign(socket, selected: selected, update_selected: selected, post: post)}
  end
  ...
  def handle_event("approve_delete", %{"value" => post_id}, socket) do
    ...
    new_page =
      if rem(Enum.count(Blog.get_all_posts()), socket.assigns.limit) == 0 and
           socket.assigns.current_page ==
             CasinosAdmins.page_count(Enum.count(Blog.get_all_posts()), socket.assigns.limit) + 1 do
  ...
  def handle_event("change_filter", %{"value" => filter}, socket) do
    ...
    posts = Blog.get_filtered_posts(filter, user)

    {:noreply, assign(socket, filter: filter, posts: posts)}
  end

  def handle_event("clear", _, socket) do
    posts = Blog.get_filtered_posts("", nil)
  ...
  def handle_event("change_author", %{"value" => "all authors"}, socket) do
    posts = Blog.get_filtered_posts(socket.assigns.filter, nil)
  ...
  def handle_event("change_author", %{"value" => author_name}, socket) do
    ...
    posts = Blog.get_filtered_posts(socket.assigns.filter, user)
    ...
    case Blog.update_publication(post, %{published_at: Blog.set_published_at(post)}) do
      {:ok, post} ->
        if post.published_at != nil do
          %{"post_id" => post.id}
          |> MySuperApp.SendPostPublicationJob.new()
          |> Oban.insert()
        end

        {:noreply, assign(socket, posts: Blog.get_all_posts())}
  ...
  def render(assigns) do
    ...
    <div class="flex gap-2 mb-3 h-full">
      <Button class="add-button justify-center" on_click="open_add_post_drawer">Add new post</Button>
    ...
    <Drawer id="add_post_drawer" is_open={@add_post_drawer_active} on_close="close_add_post_drawer">
      <Drawer.Panel>
        <Form for={@form} change="add_validate" submit="add_post">
          <div class="mx-4">
            <Field field={:title}>
              <br>
              ...
              <Input class="text-center font-normal" placeholder="Title" />
            </Field>
            <Field field={:body}>
              <br>
              <div class="text-2xl font-bold text-center mt-2 mb-2">
              ...
              <TextArea
                class="text-center font-normal w-full h-48 p-2 border border-gray-300 rounded-lg resize-y"
                placeholder="Body"
              >
              </TextArea>
            </Field>
            <Field field={:tagNames}>
              <br>
              <div class="text-2xl font-bold text-center mt-2 mb-2">
                Input tags` names
              </div>
              <Input class="text-center font-normal" placeholder="Tags" />
            </Field>
            <Field field={:user_id} class="text-moon-30 font-medium">
              <div class="text-2xl font-bold text-center mt-2 mb-2">
                User
              </div>
              <Input class="text-center" readonly placeholder={Accounts.get_user_name(@current_user.id)} />
            </Field>
            <br>
            <div class="p-2 border-t-1 border-beerus flex justify-between">
              <Button class="add-button" type="submit">Add new post</Button>
